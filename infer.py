...
def build_unpaired(image_dir: Path, cloth_dir: Path, seed: int) -> list[tuple[str, str]]:
    ...
    # Collect only persons that have a matching garment
    valid = []
    for person_path in sorted(image_dir.iterdir()):
        if not person_path.stem.endswith("_0"):
            continue
    ...
    # Shuffle until no person gets their own garment
    ...
    for _ in range(1000):
        ...
        # Guaranteed fallback: rotate by 1
...
def main():
    ...
    # Route outputs to a subfolder reflecting the mode
